...
for n = 1:numel(stats_Filter)
    ...
    % might need
    idx = ~cellfun(@isempty,regexpi(filtered_Stats{1}.Properties.VariableNames,'^(GN_Symbol|source_of_variation|contrast|pval|pval_BH|PercentChange|stratification)$'));
    ...
    idx = ~cellfun(@isempty,regexpi(total_Filtered_Stats{n}.Properties.VariableNames,strcat('^(GN_Symbol|source_of_variation|contrast|',pval_type,'|PercentChange|stratification)$')));
...
dimensional_correlation_plot(plotting_Data,save_path,pval_type);
end